...
| `appendExecution`                                 | 当前有效 activation 的 Harness         | typed 执行事件批次与 expectedSequence；工具结果采用 ToolOutcomeRef，工作区物理结果只引用已接收 Runtime receipt；领域工具引用相应提交，注册的模型侧编排可提交受验证结果，不能改写物理结果或用户票据 |
| `commitCheckpoint`                                | 当前 activation                        | checkpoint 内容/ref、覆盖 sequence、上一 checkpoint、boundary；验证所有引用已持久化及 fence 后提交；无完整恢复状态不能宣称 durable_wait                                                            |
