...
schema 3 只在**取得锁的那一次**写入，v1 不支持在持锁期间原地升级锁文件。现有 `readOwnedLock` 同时钉住 inode 与锁文件原始字节，持锁者重写自己的锁会使自持凭据立即失效，随后所有严格校验都失败。因此容器升级顺序固定为：确认无活动执行 → 记录 plan/proof → 释放旧 schema 锁 → 以 schema 3 重新取得锁 → 提交 header 与迁移完成事务。释放与重取之间的窗口由 plan/proof 和原文件前缀哈希保护，中断后由新 owner 核验恢复，不让旧二进制接管半转换文件；无法完成重取时保持旧 schema 并拒绝启用 Managed 写能力。
...
`DurableRef` 若只有 id 和 digest 就无法实现：恢复要能打开它，维护要能枚举它，删除会话要能清掉它。v1 固定如下规则。

| 事项       | v1 规则                                                                                                                                                                                                                   |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 根目录     | 与该 Session 的 transcript 同属 workspace/runtimeBaseDir 下的 `resources/<sessionId>/`；随会话目录一起备份、归档和删除，不放系统临时目录，不跨 Session 共享                                                               |
| 命名       | `resourceId` 是服务端生成的不透明 StableId，路径为 `<root>/<kind>/<resourceId>`；不接受调用方给定路径片段，不用内容 digest 命名（避免跨 Session 去重把生命周期耦合起来）                                                  |
| 枚举权威   | 权威日志中所有已提交事件与 checkpoint 的引用闭包是唯一可达性来源；目录扫描只用于发现孤儿，不用于判断某资源还需不需要                                                                                                      |
| 回收       | 仅当资源不被任何已提交事件、任何保留中的 checkpoint、任何未结算 invocation 或未消费 outbox 引用时才可删除。回收由所属 Session 的 authority 在维护操作中执行，Harness、Runtime 和客户端都不能删                            |
| 已发布孤儿 | 资源发布成功而引用它的事务未提交（见 §4 规则 2）时留下已发布孤儿。它带发布时的 owner 与 commandId；回收由同一 authority 在恢复扫描中执行——确认该 commandId 无 marker、且原 owner 已退出后删除。不设 TTL，不由旁路进程清理 |
| 归档与移动 | 归档、重命名和目录迁移必须与资源目录一起原子完成或整体失败；`resourceId` 不变，只有根目录位置改变。部分完成时保留原副本、记录未完成的维护 operation，恢复后继续，不留下引用得到但读不到的资源                             |
| 删除       | 删除会话按 §3 生命周期矩阵先到 `deleting`，排空未结算引用后删除资源目录再提交 `deleted`；有未知物理 owner 时保持 `deleting` 并保留资源，不提前删除                                                                        |
| 读取       | 只经 authority 的 registry 按 SessionKey + resourceId 解析并校验 digest/byteLength；跨 Session ref、路径穿越和 digest 不符在返回前拒绝                                                                                    |
...
- **当前 activation 的 Harness**（持 activation fence）：`model.attempt`、`message.committed`（用户输入形态由下表的输入投影适配产生）、`tool.intent`、`context.compacted`、`checkpoint.committed`，以及 `action.changed` 的 requested 分支。它经 `appendExecution`/`commitCheckpoint` 发起；authority 校验 fence 与 expectedSequence 后提交。
...
| domain.committed：domain/version/operationId/recordRef                                                                            | 注册领域适配                           | v1 的 domain 是封闭集合：`goal`、`todo`、`schedule`、`child_task`、`channel_delivery`、`history_operation`、`memory`、`session_metadata`；每个 recordRef 有独立 schemaVersion，字段由对应专项定义。新增 domain 与新增 kind 同级，按下文扩展规则处理；领域原始 payload 不展开成顶层任意事件，禁止任意 append |
...
2. 大资源先写受控临时文件，流式算 hash，完成文件同步后原子发布并同步所属目录，再允许事务引用。失败不发 ACK。注意发布早于 commit marker：事务最终未提交时留下的是**已发布**孤儿，不是临时文件，因此不能靠临时目录清理兜底——它按 §2.1 的“已发布孤儿”规则由同一 authority 在恢复扫描中按 commandId 与原 owner 核验后回收。临时文件孤儿仍只由自身 owner 回收。
...
新建 Managed 在 create 前协商 writer schema/Session reader/Runtime 控制能力。旧 legacy 从不原地改 engine；旧实验目录只提供显式位置适配，不按端口变化重新分配普通历史。已存在 managed 旧格式在无执行时可升级记录容器，顺序按 §1 固定：先记录 plan/proof，再释放旧 schema 锁并以 schema 3 重新取得，然后提交 header 与迁移完成事务；旧文件前缀保持字节一致。失败停在可核验阶段，不“修复”为 legacy。
...
| S07  | 资源仓库全生命周期：发布成功而事务未提交留下的已发布孤儿在恢复扫描后被回收且不误删仍被引用的资源；归档/重命名/移动后同 resourceId 仍可读；删除按 deleting→deleted 顺序清理，未知物理 owner 时资源保留 |
| S08  | 维护操作面对 sealed schema-3：归档/取消归档/删除/重命名/fork 正确识别封存分型或明确拒绝，绝不删锁变无锁；容器升级按释放后重取顺序执行，在释放与重取之间中断可由新 owner 核验续做，旧二进制无法接管    |
