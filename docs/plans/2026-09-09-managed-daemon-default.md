...
**尚未包含**：lock schema 3 的认证换锁——因此 Managed 会话目前仍用 schema 2 锁，尚不具备对旧二进制的防写屏障；归档/取消归档尚未按 Managed 校验。fork 与 rename 已分别由既有引擎守卫和新增 header 判定拒绝。四处普通 factory 未接线，因此 R1 在默认路径上仍等于 no-op。下一步在 R2.S1b 基础上按 R2.S2 接入完整 Harness，再执行原 R2.1～R2.4；每步保留可审查的变更和证据。
