...
**尚未包含**：R2.S2 的主体——把真实 `QwenAgent`/ACP `Session` 的读写改为经这些接缝（Session client、正式事件与 UI 投影、物理 writer 安全交接），以及 R2.S3 的持久等待与原调用恢复。lock schema 3（sealing 之上的增量加固）与 §2.1 删除状态机、跨 Session 引用/pin 保留同样未做。四处普通 factory 未接线，因此 R1 在默认路径上仍等于 no-op，daemon 尚未默认使用 Managed。下一步以 checkpoint 接缝为基础开始 R2.S2；每步保留可审查的变更和证据。
