...
**坏尾恢复已交付**（存储规范 §4 规则 6）：崩溃在事件与 marker 之间会留下从未提交的尾部，原先该会话永久不可写。新增 `SessionWriterLease.truncateTo(byteLength)` 在同一 `runExclusive` 屏障与逐次归属校验下截断，并重读保留前缀重建被钉住的证明（滚动 hash 不能回退）。authority 侧 `recoverUncommittedTail` 显式发起：确认存在无 marker 尾部与完整前缀后，先把被丢弃字节存成 `<transcript>.uncommitted-tail` 诊断副本再截断。`open()` 仍报错拒写，修复不会被打开会话或兼容探测隐式触发。证据：4 项定向单测，含以真实 lease 追加无 marker 事件模拟崩溃后「open 拒绝 → 恢复 → 重开只见已提交前缀 → 继续追加序号 3/4 → 冷重开确认」的完整链路（若证明未正确重建，后续追加或冷重开会以 `SessionTranscriptChangedError` 失败），以及诊断副本字节数核对与超界/负数截断拒绝。`session-writer-lease` 原有套件与 `managed-runtime`、`chatRecordingService`、`sessionService` 合计 594 项通过。

**尚未包含**：lock schema 3 的认证换锁——因此 Managed 会话目前仍用 schema 2 锁，尚不具备对旧二进制的防写屏障；归档/取消归档/删除/重命名/fork 的维护路径尚未按 Managed 适配。四处普通 factory 未接线，因此 R1 在默认路径上仍等于 no-op。下一步在 R2.S1b 基础上按 R2.S2 接入完整 Harness，再执行原 R2.1～R2.4；每步保留可审查的变更和证据。
