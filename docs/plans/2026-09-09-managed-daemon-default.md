...
**尚未包含**：R2.S2 的其余部分——把 `ChatRecordingService`、core client、llm-chat、工具调度器、Session 终态、Goal/记忆与文件历史等**真实写入点**改为经该投影与 authority 提交（目前投影本身没有生产调用者），以及 UI 投影与物理 writer 安全交接；R2.S3 的持久等待与原调用恢复。lock schema 3（sealing 之上的增量加固）、§2.1 删除状态机与跨 Session 引用/pin 保留同样未做。四处普通 factory 未接线，因此 R1 在默认路径上仍等于 no-op，daemon 尚未默认使用 Managed。下一步把上述写入点逐个迁到该接缝；每步保留可审查的变更和证据。
