...
**尚未包含**：lock schema 3 的认证换锁；`sessionService.ts` 的同步标题读取仍按 `"subtype":"custom_title"` 子串匹配，对 Managed transcript 返回空标题，这是默认启用前必须结清的项；§4 规则 6 的坏尾截断需要新的 lease 能力（见存储规范该条实测补充），在此之前无 marker 的尾部保持 blocked。四处普通 factory 未接线，因此 R1 在默认路径上仍等于 no-op。下一步在 R2.S1b 基础上按 R2.S2 接入完整 Harness，再执行原 R2.1～R2.4；每步保留可审查的变更和证据。
